...
function RetCampoCid(aTab:string) : string;
begin
  if aTab = 'CLIENTES' then Result := 'CIDADE'
  else if aTab = 'FORNECEDORES' then Result := 'CIDADE'
end;
...
begin
  ...
  if (rgpSelecione.ItemIndex=2) and (cmbTabela.Items[cmbTabela.ItemIndex]='Produtos') then
  begin
    MessageDlg('Pesquisa incompatível',mtInformation,[mbOk],0);
    Exit;
  end;
  ...
  case rgpSelecione.ItemIndex of
    ...
    2: vSQL := vSQL + #13 + ' WHERE ' + RetCampoCid(nmTab)   + ' CONTAINING :PCD';
  end;
